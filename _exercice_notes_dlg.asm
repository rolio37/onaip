...
exercice_notes_dlg_go proc
 ...
 ; récupérer les altérations choisi par l'utilisateur
 ; diese
 ...
 .if eax == 0
  ...
  ; 1 octave
  mov endc.touche_aleatoire_max, 12 + 1 ; + 1 pour le gros problème d'arrondi du processeur intel atom, BIZARRE ? 
...
exercice_notes_dlg_stop proc
 ...
 ; réafficher la fenêtre d'exercice surement mise de côté
...
exercices_notes_aleatoire proc
 ...
 .else
  ...
  .if eax == endc.clef2
   ...
   ; choisir la portée au hasard
   ; choisir un des deux aléatoirements
   invoke GetSystemTime, addr sts
   mov cx, sts.wMilliseconds
   rdtsc
   rcr eax, cl
 ...
 ; *************************************************
  ...
  .else
   ...
   ; aucune note accidentelle dièse ou bémol
   ; supprime l'atération de la note aléatoire si besoin
 ...
 ; définir la note sur la partition
...
exercices_notes_dlg_gestion_reponse proc nb_events:dword, buffer_event:dword
 ...
 .if  t_event == MIDI_TOUCHE_APPUYEE
  ...
  .else
   ...
   ; erreur afficher la note en rouge erroné et laisser celle d'origine en noire
